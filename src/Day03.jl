...
const part2_slopes = [
            CartesianIndex(1, 1), CartesianIndex(3, 1), 
            CartesianIndex(5, 1), CartesianIndex(7, 1), 
            CartesianIndex(1, 2)]

function solve(i::AbstractString = input, start::CartesianIndex = start_pos,
           slope = part1_slope, slopes = part2_slopes
              )
...
function parsemap(data)
    ...
    return terrain = mapreduce(parserow, hcat, data)
end
...
function count_trees(terrain::AbstractArray, start, slope)
    wrap = size(terrain)[1]
